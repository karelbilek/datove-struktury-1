...
klíčů při neúspěšném vyhledávání při operaci {\bf INSERT$(s_{n+1})$.} 
\end{lemma}
\begin{dukaz}
     Jednička je za poslední porovnání $s_{n+1}$.
\end{dukaz}

\begin{pozorovani}
Když $
s_{n+1}$ je vložen 
na místo $h(s_{n+1})$, nebyl porovnáván žádný klíč a test pro úspěšné vyhledávání bude $
1$.
\end{pozorovani}
...
l$, pak bylo 
při operaci {\bf INSERT$(s_{n+1})$} použito $l-i+1$ porovnání klíčů (musel jsem \uv{projet} celý zbytek řetězce)
a teď se použije $l-i+2$ testů.
\end{pozorovani}

 Podle předchozí části analýzy dostaneme, že očekávaný počet 
