...
\flushpar V praxi se často setkáváme s následujícím 
...
\flushpar Zadání problému: Nechť $U$ je univerzum. Je dána množina 
...
\flushpar\emph{Halda} je stromová struktura, kde vrcholy 
...
\flushpar Probereme několik verzí hald a budeme 
...
\flushpar {\bf MAKEHEAP$(S,f)$} -- operace vytvoří haldu 
...
\centerline{\bigbigrm{II. Regulární haldy}}
\bigskip

\flushpar První použité haldy byly binární neboli $
2$-regulární haldy. 
Tyto haldy jsou velmi oblíbené pro svou jednoduchost a 
názornost a pro velmi efektivní implementaci.
\medskip

\flushpar Předpokládejme, že $d>1$ je přirozené číslo. 
...
\proclaim{Tvrzení}Každý $d$-regulární strom má nejvýše jeden 
...
\demo{Důkaz}První část tvrzení plyne přímo z požadavku 2) na 
...
\flushpar Všimněme si, že speciálně pro $d=2$ mají synové vrcholu s číslem $
k$ 
...
\flushpar\v Rekneme, že množina $S$ s funkcí $f$ je reprezentována 
...
\flushpar Defince $d$-regulárního stromu umožňuje velmi efektivní 
...
\flushpar Pro $d$-regulární haldy není známa efektivní implementace operace 
...
}\flushpar Ověříme korektnost algoritmů.  Je zřejmé, že 
...
\flushpar Vypočteme časovou složitost operací:  Jeden běh cyklu 
...
\flushpar Třídění: prostou posloupnost 
...
\flushpar Teoreticky lze ukázat, že použití $d$-regulárních hald v 
...
\flushpar Dalším příkladem je nalezení nejkratších cest v grafu z daného 
...
}\flushpar Korektnost algoritmu je založena na kombinatorickém 
...
\flushpar Dalším typem hald, se kterými se seznámíme, jsou lefist 
...
\flushpar Mějme binární kořenový strom $(T,r)$ (to znamená, že $
r$ je kořen, 
...
\flushpar Mějme $S\subseteq U$ a funkci $f:S@>>>\Bbb R$. Pak binární strom 
$(T,r)$ takový, že
\roster
\item
...
\flushpar Struktura vrcholu $v$ v leftist haldě:\newline 
S vrcholem $v$ jsou 
...
\flushpar Uvedeme základní vlastnost leftist haldy, která 
...
\proclaim{Tvrzení}V leftist haldě je 
...
\flushpar Základní operací pro leftist haldy je {\bf MERGE}. Tato 
...
}\flushpar Vypočteme časovou složitost předchozích 
algoritmů. Každý běh 
...
\flushpar Implementace operací {\bf DECREASE} a {\bf INCREASE} pomocí 
...
je pak použit i pro Fibonacciho haldy.  \medskip

\flushpar Nejprve popíšeme pomocnou operaci {\bf Oprav$(T,v
)$}, 
...
}\flushpar Po provedení operace {\bf Oprav} mají všechny vrcholy 
...
\flushpar Popíšeme ostatní algoritmy.
\bigskip
...
}\flushpar Protože algoritmy {\bf MERGE} a {\bf Oprav} vyžadují čas 
...
\proclaim{Věta}V leftist haldách existuje implementace operace 
...
\centerline{\bigbigrm{IV. Amortizovaná složitost}}
\medskip

\flushpar Popíšeme bankovní paradigma pro počítání s 
...
\flushpar Obvykle je $h(D)\ge 0$ pro všechny konfigurace $D$ 
...
\centerline{\bigbigrm{V. Binomiální haldy}}
\medskip

\flushpar Další typ hald je motivován sčítaním přirozených čísel.  
...
\flushpar Pro $i=0,1,\dots$ definujeme rekurentně binomiální 
...
\flushpar Nejprve uvedeme základní vlastnosti těchto 
stromů.
\medskip

\proclaim{Tvrzení}Pro každé přirozené číslo $
i=0,1,\dots$ 
platí:
\roster
\item
...
\demo{Důkaz}Tvrzení platí pro strom $H_0$ a 
...
\flushpar\emph{Binomiální} \emph{halda} $\Cal H$ reprezentující 
...
\flushpar Z binárního zápisu přirozených čísel plyne, že 
...
\flushpar Operace pro binomiální haldy jsou  
...
\flushpar Je vidět, že když oba stromy $T_1$ a $T_2$ jsou izomorfní 
...
}\flushpar Z podmínky \thetag{usp} je zřejmé, že 
...
\flushpar Výpočet časové složitosti operací pro binomiální haldy 
...
\flushpar Z tvrzení také plyne, že výška všech stromů v 
...
\proclaim{Věta}Pro binomiální haldy algoritmy operací 
...
\flushpar Z těchto výsledků je vidět, že předchozí 
...
\flushpar Následující algoritmy jsou založeny na ideji, že 
...
\flushpar\emph{Líná} \emph{binomiální} \emph{halda} $\Cal H$ 
...
\flushpar V líné binomiální haldě je vynechán předpoklad 
...
\flushpar Samotný algoritmus pro operaci {\bf INSERT} se nezmění, jen 
...
\flushpar}\v Casová složitost operací {\bf INSERT} a {\bf MER\-GE} při líné 
...
\flushpar Protože si funkci ohodnocení volíme, 
...
\flushpar Význam Fibonacciho hald určuje fakt, že amortizovaná 
...
\flushpar Neformálně řečeno, je Fibonacciho halda množina 
...
\flushpar V algoritmech předpokládáme, že Fibonacciho halda je 
...
}\flushpar Všimněme si, že když stromy $T_1$ a $T_2$ mají rank 
...
\flushpar Naším cílem bude odhadnout amortizovanou složitost těchto 
...
\flushpar {\bf MERGE} -- časová složitost $O(1)$, nevzniká žádný 
nový strom, 
...
\flushpar Pro výpočet amortizované složitosti 
...
označených vrcholů.
\medskip
\flushpar Nechť $\rho (n)$ je maximální 
...
\flushpar Vezměme nyní nejmenší $i$ takové, že $
n<F_i$. Protože 
...
\flushpar Výsledky shrneme do následující věty: 
\medskip

\proclaim{Věta}Ve Fibonacciho haldě, která   
...
\flushpar Pro úplnost dokážeme, že $F_i=\frac {\big(\frac {
...
\flushpar Vrátíme se k Dijkstrově algoritmu.  
...
\flushpar Otázka je, kdy v Dijkstrově algoritmu nebo v 
...
\flushpar Binární neboli $2$-regulární haldy 
...
\centerline{\bigbigrm{VII. Třídicí algoritmy}}
\bigskip

\flushpar Jednou z nejčastěji řešených úloh při práci s 
daty je setřídění 
...
\flushpar Základní algoritmy, které  řeší třídicí 
...
\flushpar S algoritmem {\bf HEAPSORT} jsme se seznámili při 
...
\flushpar Třídicí algoritmy se často používají jako 
...
\flushpar {\bf HEAPSORTU} je stále věnována velká pozornost a 
...
\flushpar Nejstarší z uvedených algoritmů je 
...
\flushpar Všimněme si, že všechny posloupnosti v $Q$ jsou 
...
\flushpar Spočítáme časovou složitost {\bf MERGESORTU}. 
...
\flushpar Nyní vypočteme složitost hlavní procedury. 
...
\flushpar Nyní popíšeme patrně vůbec 
...
}\flushpar Algoritmus {\bf Quick} setřídí posloupnost 
...
\flushpar Procedura {\bf Quick} bez rekurzivního volání 
...
\flushpar Protože při každém volání má {\bf Quick} jako 
argument kratší 
...
\flushpar Následná analý\-za očekávaného případu  
...
\flushpar Ukážeme dva způsoby výpočty očekávaného času.  Jeden 
...
\flushpar Nechť $(b_1,b_2,\dots,b_n)$ je výsledná posloupnost.  
...
\flushpar Abychom spočítali $p_{i,j}$, popíšeme chování algoritmu 
...
\flushpar Očíslujeme vrcholy tohoto stromu prohledáváním do 
...
\flushpar Nyní porovnáme složitost algoritmů 
...
\flushpar {\bf QUICKSORT} v nejhorším případě vyžaduje čas 
...
\flushpar Prezentované  výsledky byly spočítány 
pro model RAM (viz Mehlhorn 1984).
\medskip

\flushpar Očekávaný čas pro {\bf HEAPSORT} je prakticky stejný jako 
...
\flushpar V algoritmu {\bf MERGESORT} jsme použili frontu, která 
...
\flushpar Vstup: Množina rostoucích navzájem 
...
\flushpar Předpokládejme, že máme postup, který z 
...
\flushpar Když tedy $T$ je úplný binární strom, jehož listy
...
}\flushpar Nyní můžeme přeformulovat původní 
problém:\newline 
...
\flushpar V přeformulováné úloze už nepracujeme s 
...
\flushpar Mějme množinu $\{x_i\mid i=1,2,\dots,n\}$.  Pro úplný binární 
...
\flushpar Vytvoření nového stromu $v$ spojením stromů 
...
\proclaim{Věta}Pro danou posloupnost čísel 
...
\flushpar Předpokládejme opět, že $x_1\le x_2\le\dots\le 
x_n$ a že v daném 
...
\flushpar Pro aplikaci na naši původní úlohu je 
třeba  
...
\proclaim{Věta}Uvedený algoritmus množinu 
...
\centerline{\bigbigrm{VIII. Rozhodovací stromy}}
\medskip

\flushpar Většina obecných třídicích algoritmů 
...
\flushpar V nevhodném algoritmu se může stát, že 
...
\flushpar Následující obrázek ilustruje naše úvahy na 
...
\flushpar Pak korektnost algoritmu zajišťuje, že dvě různé 
...
\flushpar Když nejdelší cesta z kořene do listu v 
...
\flushpar Dále pro binární strom $T$ označme $B(T)$ součet všech 
...
\proclaim{Věta}Každý třídicí algoritmus, jehož jedinou 
...
\centerline{\bigbigrm{IX. Přihrádkové třídění}}
\bigskip


\flushpar V následujících algoritmech 
...
\flushpar Algoritmus nevyžaduje, aby prvky ve 
...
\flushpar Nyní uvedeme dvě sofistikovanější verze tohoto 
...
\proclaim{Věta}Algoritmus {\bf HYBRIDSORT} setřídí 
...
\demo{Důkaz}První dva cykly v algoritmu 
...
\flushpar Nyní odhadneme očekávaný čas. Položme 
$X_i=|Q_i|$. Pak 
...
\flushpar Poznámka: V důkazu jsme použili odhad 
...
\flushpar Nyní použijeme modifikaci {\bf BUCKETSORTU} pro 
...
\flushpar Uvažujme jeden běh posledního cyklu algortimu 
pro určité $i$. Po 
...
\flushpar Označme $L=\sum_{i=1}^nl(i)$ a připomeňme, že 
...
\centerline{\bigbigrm{X. Pořádkové statistiky}}
\bigskip

\flushpar Na závěr popíšeme dva algoritmy pro hledání $
k$-tého 
...
\flushpar Pracujeme s totálně uspořádaným univerzem $
U$.\newline 
...
\flushpar Důkaz korektnosti algoritmu je založen na následujícím 
...
\proclaim{Věta}Algoritmus {\bf FIND} nalezne $i$-tý nejmenší 
...
\flushpar Pro velmi malá $i$ nebo pro $i$ velmi blízká $n$ 
...
\flushpar Následující algoritmus nalezne $i$-tý nejmenší 
...
}\flushpar Důkaz korektnosti algoritmu je stejný 
jako u algoritmu {\bf FIND}. 
Zbývá vyšetřit složitost. Nej\-prve dokážeme následující 
lemma.
\medskip

\proclaim{Lemma}Když $n\ge 100$, pak $|M_1|,|M_2|\le\frac {8n}{
11}$.
\endproclaim

\demo{Důkaz} Pro $j\le\lfloor\frac n5\rfloor$ platí, že když $
m_j<\bar {m}$, pak 
...
\flushpar Maximální čas vyžadovaný algoritmem  
...
\proclaim{Věta}Algoritmus {\bf SELECT} nalezne $i$-tý nejmenší 
prvek v lineárním čase.
\endproclaim
\medskip

\flushpar Algoritmus {\bf FIND} je ve velké většině 
případů 
