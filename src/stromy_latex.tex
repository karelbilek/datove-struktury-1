...
\flushpar Jedná se o rozšíření základního slovníkového problému. Je 
...
\flushpar Je vidět, že operace {\bf JOIN2} a {\bf JOIN3} lze pomocí ope\-rací 
...
\flushpar Důležitou datovou strukturou vhodnou pro 
...
\flushpar Výhody našich $(a,b)$-stromů:\newline 
...
\proclaim{Tvrzení}Mějme přirozená čísla $a$ a $
b$ taková, 
...
\flushpar Mějme kořenový strom $(T,t)$ takový, že pro každý 
...
\flushpar Předpokládáme, že v $(a,b)$-stromu  
...
\flushpar Mějme lineárně uspořádané univerzum $U$ a 
...
\flushpar Struktura vnitřních vrcholů 
...
\flushpar Struktura listů:\newline 
listu $v$ je přiřazen prvek $\key(v)\in S$.
\medskip

\flushpar Někdy je ve struktuře každého vrcholu $v$ 
$(a,b)$-stromu různého od kořene ještě ukazatel 
$\otec(v)$ na otce vrcholu $v$.
\medskip

\flushpar Když $H_v(i)$ jsou prvky z reprezentované 
...
\flushpar Nyní uvedeme algoritmy pro $(a,b)$-stromy.

\subhead 
Algoritmy
\endsubhead

\flushpar Pomocný algoritmus
\bigskip
...
\flushpar Poznámky k algoritmům. \newline 
...
\flushpar Při operaci {\bf SPLIT} se zásobníky používají 
...
\endroster
\flushpar Toto plyne z první fáze algoritmu operace {\bf SPLIT} a zajišťuje 
korektnost druhé fáze algoritmu. 
\medskip

\flushpar Dále si všimněme, že podprocedury {\bf Štěpení}, {\bf Spojení} a {\bf Pře\-sun }
...
\proclaim{Věta}Algoritmy pro operace {\bf MEMBER}, {\bf INSERT}, 
...
\flushpar Je třeba ještě odhadnout spotřebovaný čas ve druhé fázi 
...
\flushpar Nyní popíšeme algoritmus pro operaci {\bf ord$
(k)$}. Tato 
...
\flushpar Udržovat pole $P_v$ v aktuálním stavu znamená při úspěšném 
...
\flushpar Invariant algoritmu: V každém okamžiku platí, 
...
\flushpar $(a,b)$-stromy se používají jak v interní tak v 
...
\flushpar Když je množina reprezentovaná $(a,b)$-stromem 
...
\flushpar Při operaci {\bf DELETE} jsou ve vyhledávací fázi uzavřeny 
...
\flushpar Tato úprava vyžaduje sice více {\bf Štěpení}, {\bf Spojení} a
...
\flushpar Operace {\bf JOIN2} lze také paralelizovat, ale operaci {\bf SPLIT  }
paralelizovat nelze.
\medskip

\flushpar$(a,b)$-stromy dávají také zajímavé aplikace pro 
třídicí 
...
\flushpar Modifikace $(a,b)$-stromů pro algoritmus 
...
\flushpar Korektnost algoritmu plyne z faktu, že $\key$ je 
...
\flushpar Složitost algoritmu: Algoritmus $A$-{\bf sort} vyžaduje více 
...
\flushpar Zřejmě algoritmus $A$-{\bf sort v} nejhorším případě vyžaduje čas, 
...
\flushpar Proto v nejhorším případě čas potřebný pro nalezení 
pozice $x_i$ je $O(\log f_i)$. Odtud plyne, že čas algoritmu potřebný k 
běhu algoritmu $A$-{\bf sort} je 
$$O((\sum_{i=1}^n\log f_i)+n).$$
\medskip

\flushpar Zřejmě $\sum_{i=1}^nf_i=F$ a nyní využijeme toho, že geometrický 
...
\proclaim{Věta}Algoritmus $A$-{\bf sort} na setřídění $
n$-členné 
...
\flushpar Zhodnocení: Protože $A$-{\bf sort} nepoužívá operaci 
...
\flushpar\emph{Hladinově} \emph{propojený} $(a,b)$-\emph{strom} \emph{s} 
...
\flushpar Zde se liší hlavně vyhledávání, které je zobecněním 
...
\flushpar Struktura kromě operací uspořádaného 
...
\flushpar Použití: Tato struktura je velmi výhodná pro 
...
\flushpar Vyvažovací operace {\bf Štěpení, Spojování, Přesun }
...
\flushpar Pro pevné $a$ a $b$ označme 
...
\proclaim{Věta}Nechť $b\ge 2a$ a $a\ge 2$. Nechť $\Cal P$ je 
...
\flushpar Z definice plyne, že $c\ge 1$, a protože $a\ge 2$, z 1) dostaneme
...
\flushpar Důkaz je založen na bankovním principu -- 
...
\flushpar Nyní rozložíme operace {\bf INSERT} a {\bf DELETE} do 
...
\proclaim{Pozorování}Mějme dva stromy $T$ a $T'$, které mají 
...
\proclaim{Lemma 1}Když $(T,r)$ je $(a,b)$-strom a když strom $
T'$ 
...
\proclaim{Lemma 2}Nechť $(T,r,v)$ je parciální $(a,b)$-strom, 
...
\proclaim{Lemma 3}Nechť $(T,r,v)$ je parciální $(a,b)$-strom, 
...
\proclaim{Lemma 4} Nechť $(T,r,v)$ je parciální $(a,b)$-strom, 
...
\flushpar Označme $T_k$ $(a,b)$-strom vzniklý provedením posloupnosti 
$\Cal P$ na prázdný $(a,b)$-strom. Sečtením předchozích 
výsledků dostá\-váme
\medskip

\proclaim{Důsledek 5}Když položíme 
...
\flushpar První výraz upravíme (využíváme, že $
c\ge 1$):
...
\flushpar Nyní odhadneme shora $b(T_k)$.
\medskip

\proclaim{Lemma 6}Když $T$ je $(a,b)$-strom s $m$ listy, pak 
$0\le b(T)\le c+(m-2)\frac c{a+c-1}$.
\endproclaim

\demo{Důkaz}Pro $0\le j<c$ označme $m_j$ počet 
...
\flushpar Nyní dokážeme tvrzení (1) Věty. Protože každá operace 
...
\flushpar Důkaz (2) využije následující odhad.
\medskip

\proclaim{Lemma 7}Pro každé $h\ge 1$ a pro každý 
$(a,b)$-strom $T$ s $m$ listy platí 
$$\sum_{l=1}^hb_l(T)(c+1)^l\le (c+1)m.$$
\endproclaim

\demo{Důkaz}Pro $0\le j<c$ a pro libovolné $h$ 
...
\flushpar Věta vysvětluje, proč jsou doporučené hodnoty $
b\ge 2a$ -- 
...
\proclaim{Věta}Předpokládejme, že $b\ge 2a$ a $a\ge 2$. Mějme 
...
\flushpar Vysvětlení: Začínáme v libovolném 
...
\flushpar Aplikace: analýza hladinově propojených stromů s prstem 
...
\flushpar Vyvažování při operaci {\bf INSERT} lze provádět 
...
\flushpar Zadání úlohy: Máme podmnožinu $S$ lineárně 
...
\flushparŘešení:  Pokud $x<A(1)$ nebo $A(|S|)<x$, pak $
x$ 
...
\flushpar V tomto metaalgoritmu je {\bf next$(d,h)$} funkce, která 
...
\flushpar Unární vyhledávání: {\bf next$(d,h)=d+1$}, pak každý 
...
\flushpar Poznámka: Duální přístup je, když 
...
\flushpar Binární vyhledávání: {\bf next$(d,h)=\lceil\frac {
d+h}2\rceil$}, pak 
...
\flushpar Interpolační vyhledávání: 
...
\flushpar Po dvou dotazech klesne $h-d$ buď pod $\sqrt {h-d}$ nebo pod 
...
\flushpar Nyní spočítáme očekávaný počet dotazů 
...
\flushpar Uvažujme okamžik, kdy jsme na začátku nějakého 
...
\flushpar Když $E(T(n))$ je očekávaný počet dotazů pro operaci 
...
\proclaim{Věta}Čas operace {\bf MEMBER} v uspořádaném poli délky $
n$ při 
...
\flushpar Nevýhoda této datové struktury 
...
\flushpar Binární vyhledávací strom je struktura pro binár\-ní 
vyhledávání v uspořáda\-ném poli rozta\-ženém do roviny a 
vyhledávání odpo\-ví\-dá cestě ve stro\-mě. Formální 
definice:

\flushpar Předpokládáme, že $U$ je lineárně uspořádané 
...
\flushpar Strukura vnitřního vrcholu $v$:\newline 
...
\flushpar Každý list reprezentuje interval mezi dvěma 
...
\flushpar Při implementaci binárních vyhledávácích 
...
\flushpar Navrhneme algoritmy pro binární vyhledávací stromy 
...
\flushpar Abych dokázali korektnost algoritmu {\bf Vyhledej} -- 
...
\proclaim{Lemma}Je-li $T'$ podstrom binárního vyhledávacího 
...
\demo{Důkaz}Tvrzení dokážeme indukcí.  Zřejmě platí, 
...
\flushpar Korektnost operací {\bf MIN}, {\bf MAX} a {\bf JOIN3} plyne z definice 
...
\flushpar Zpracování jednoho vrcholu vyžaduje čas $O(
1)$ a 
...
\proclaim{Věta}Algoritmy pro operace {\bf MEMBER}, {\bf INSERT}, {\bf DE\-LE\-TE}, {\bf MIN}, {\bf MAX}, 
{\bf JOIN3} a {\bf SPLIT} v binár\-ním vy\-hledá\-va\-cím stro\-mě $
T$ vyžadují 
čas $O(\hloubka(T))$. 
\endproclaim

\flushpar Bohužel ani struktura binárních vyhledávacích 
...
\flushpar Korektnost algoritmu plyne z následujícího invariantu:  
...
\flushpar Tento výsledek motivuje používání binárních 
...
\flushpar Při náhodné posloupnosti operací {\bf INSERT} a {\bf DELETE} je 
...
\flushpar Studují se tzv.  samoupravující struktury.  Zde se 
...
\flushpar Nyní si ukážme dvě operace se stromy, na nichž jsou 
založeny  vyvažovací operace pro binární vyhledávací 
stromy. Obě ope\-race vyžadují čas $O(1)$.
\medskip

\flushpar Mějme vrchol $v$ binárního vyhledávacího stromu $
T$ a 
...
\flushpar Všimněme si, že při {\bf Rotace} můžeme 
...
\flushpar Mějme vrchol $w$ stromu $T$, jeho syna $v$ a jeho 
...
\flushpar Také zde můžeme v čase $O(1)$ spočítat nové hodnoty $
p$. 
...
\flushpar Binární vyhledávací strom je 
...
\flushpar Pro vnitřní vrchol $v$ stromu $T$ označme $\eta 
(v)$ 
délku nejdelší cesty z vrcholu $v$ do listu.
\medskip
 
\flushpar Struktura vnitřních vrcholů v 
...
\flushpar Všimněme si, že hodnota $\eta (v)$ pro vnitřní vrcholy 
...
\flushpar Odhad velikosti $\eta (\text{\rm kořen }T)$ v závislosti na 
...
\flushpar Dokážeme, že $mx(i)=2^i-1$. Tento vzorec je 
splněn pro $i=1,2$. Dále 
$$mx(i+1)=2mx(i)+1=2(2^i-1)+1=2^{i+1}-1.$$
Tím je vzorec dokázán. 
\medskip

\flushpar Abychom spočítali $mn$, připomeneme si definici Fibonacciho čísel. \emph{Fibonacciho} 
...
\flushpar Dokážeme, že $mn(i)=F_{i+2}-1$. Protože $F_3
=2$ a 
$F_4=3$, 
...
\flushpar Když AVL-strom $T$ o výšce $i$ reprezentuje množinu $
S$ o 
...
\flushpar Operace {\bf MEMBER$(x)$} pro AVL-stromy je stejná jako 
...
\flushpar Mějme vrchol $t$, jeho $\eta (t)=a$ (ale $a$ neznáme), na 
začátku 
...
\proclaim{Lemma}Když se hodnota $\eta (t)$ při operaci {\bf INSERT }
zvětšila a $t$ nebyl listem před operaci, pak po operaci 
neplatí $\omega (t)=0$.
\endproclaim

\flushpar Označme $u=\pravy(v)$ a uvažme případy:\newline 
...
\flushpar Všimněme si, že po provedení {\bf Rotace} nebo
...
\flushpar Popíšeme vyvažovací operaci pro operaci 
...
\flushpar V analýze je důležité, že když procedura {\bf Kontrola-DELETE }
...
\flushpar Dán vrchol $t$, jehož hodnota $\eta (t)$ se zmenšila (o $
1$).  V 
...
\flushpar V operaci {\bf DELETE} se může stát, že procedury 
...
\proclaim{Věta}Datová struktura AVL-strom 
...
\flushpar Binární vyhledávací strom $T$ reprezentující 
...
\flushpar Nejprve ukážeme, že červeno-černé stromy jsou 
...
\proclaim{Tvrzení}Když červeno-černý strom $T$ 
reprezentuje množinu $S$, pak $$\hloubka(T)\le 2\log(2|S|+2)=1+\log(|S|+1).$$
\endproclaim

\flushpar Pro červeno-černé stromy navrhneme algoritmy 
...
\flushpar Nejprve popíšeme vyvažovací operace. 
...
\flushpar Vyvažování 2-parciálního červeno-černého stromu $(T',v)$ 
...
\flushpar Obarvení je realizováno rozšířením struktury vrcholu $v$ 
o boolskou proměnnou $b(v)$, kde $b(v)=0$ znamená, že $v$ je obarven 
červeně, a $b(v)=1$ znamená, že $v$ je obarven černě.
\medskip

\flushpar Popíšeme proceduru {\bf Vyvaz-INSERT$(v)$} (předpokládáme, že 
...
\flushpar Na obrázku $b$ značí černou barvu a $r$ značí červenou 
...
\flushpar$2$-parciální červeno-černé stromy vznikají při 
...
\flushparŘekneme, že dvojice $(T,v)$ je \emph{3}-\emph{parciální} 
...
\flushpar Popíšeme proceduru {\bf Vyvaz-DELETE$(v)$}, která se 
...
\flushpar V následujících obrázcích jsou vrcholy, které 
...
\flushpar Nyní popíšeme algoritmy realizující operace {\bf INSERT}, 
...
\flushpar Operace {\bf INSERT} v červeno-černých stromech 
volá nejvýše $2+\log(|S|)$-krát proceduru {\bf Vyvaz-INSERT} a 
provede nejvýše jednu rotaci nebo dvojitou rotaci.
\medskip

\flushpar Operace {\bf DELETE} je řešena stejným způsobem 
...
\flushpar Mějme červeno-černé stromy $T_1$ a $T_2$ reprezentující 
...
\flushpar Algoritmus pro operaci {\bf SPLIT} je velmi podobný 
...
\flushpar Nyní popíšeme algoritmy pro tyto operace.
\medskip
...
\flushpar Korektnost algoritmů je vidět z 
...
\proclaim{Věta}Algoritmy operací {\bf MEMBER}, {\bf INSERT}, 
...
\flushpar Všimněte si, že operace {\bf JOIN3} ve skutečnosti 
...
\flushpar Pokud chceme mít i algoritmus pro operaci {\bf ord$
(k)$}, pak 
...
\proclaim{Věta}Algoritmy operací {\bf MEMBER}, 
...
\flushpar Vzniká otázka, proč se tolik pozornosti věnuje 
...
\flushpar V osmdesátých letech se ve výpočetní geometrii 
...
\proclaim{Tvrzení}Když $T$ je $BB(\alpha )$-strom reprezentující 
$n$-prvkovou množinu, pak 
$$\hloubka(T)\le 1+\frac {\log(n+1)-1}{\log\frac 1{1-\alpha}}.$$
\endproclaim

\flushpar Důsledek je, že $BB(\alpha )$-stromy patří do 
...
\proclaim{Tvrzení}Pro každé $\alpha$ existuje konstanta $
d$ 
...
\flushpar Toto tvrzení a jeho symetrické verze jednoznačně ukazují, 
jak vyvažovat $BB(\alpha )$-stromy při aktualizačních 
operacích (podstrom $BB(\alpha )$-stromu je $BB(\alpha )$-strom). Pak dostáváme:

\proclaim{Věta}Implementace operací {\bf MEMBER}, {\bf INSERT} a {\bf DELETE }v $BB(\alpha )$-stromech vyžaduje v nejhorším 
případě čas $O(\log(|S|))$, kde $S$ je 
reprezentovaná množina.
\endproclaim

\flushpar Obliba $BB(\alpha )$-stromů byla zapříčiněna 
platností následující věty, která je analogií věty o 
vyvažovacích operacích pro $(a,b)$-stromy.

\proclaim{Věta}Když $\alpha$ je reálné číslo takové, že 
...
\smallskip
\flushpar Analýza interpolačního vyhledávání pochází od Perla, 
Itai a Avniho (1978),\newline 
