...
\flushpar Základní problém: Reprezentace množin a 
...
\flushpar Cílem této přednášky není pouze seznámit vás s algoritmy 
...
\flushpar\v Rešíme tzv. slovníkový problém: Dáno 
...
\flushpar Pomocí bitového pole můžeme rychle 
implementovat operace {\bf MEMBER}, {\bf INSERT} a {\bf DELETE}. 

\flushpar Nevýhoda: když je velké univerzum, pak je prostorová 
...
\flushpar Základní idea: Dáno univerzum $U$ a množina 
...
\flushpar Příklad: $U=\{1,2,\dots,1000\}$, $S=\{1,7,11,53,
73,141,161\}$ a 
...
\flushpar V následující analýze předpokládáme, že hodnota funkce $
h(x)$ 
...
\flushpar Spočítáme očekávanou délku řetězců za 
předpokladů
\roster
\item
...
\endroster
\flushpar Vysvětlení: $i$-tý řetězec má délku $
...
\flushpar Zde jsme nepřesní. Obecně pro náhodně zvolené $
x\in U$ 
...
\flushpar Předpoklad: $\alpha =\frac nm\le 1$. Ukážeme, že pro dostatečně 
velká $n$ pro 
...
\flushpar Když $0.5\le\alpha\le 1$, je to zároveň i dolní odhad.
\endproclaim
...
\flushpar Test je porovnání argumentu operace s 
prvkem na daném místě řetězce nebo zjiště\-ní, 
že vyšetřovaný řetězec je prázdný.
\medskip

\flushpar Budeme rozlišovat dva případy:\newline 
...
\flushpar Očekávaný počet testů:
...
\flushpar Jiný postup: Předpokládejme, že počet testů při úspěšném 
...
\flushpar Následující tabulka dává přehled očekávaného 
...
\flushpar Všimněte si, že očekávaný počet testů 
...
\flushpar Velikost $\alpha$ je doporučována menší než $
1$, ale nemá být 
...
\flushpar Vylepšení metody: hašování s uspořádanými řetězci.
...
\flushpar Uvedeme algoritmy pro operace s uspořádanými 
ře\-těz\-ci.
\medskip
...
}\flushpar Nevýhody hašovaní se separovanými řetězci --\newline 
\phantom{---}nevyužití alokované paměti (nehospodárné)\newline 
\phantom{---}používání ukazatelů (cache).\newline 

\flushpar\v Rešení: využít pro řetězce původní tabulku. Pak řádky tabulky musí mít strukturu, která umožňuje prohledávat řetězce, a velikost reprezentované množiny 
může být nejvýše rovna velikosti tabulky.  
\medskip
\flushpar Položky tabulky:\newline 
\phantom{---}key,\newline 
\phantom{---}odkaz na uložená data,\newline 
\phantom{---}položky pro práci s tabulkou.\medskip

\flushpar Předpokládáme, že data jsou velká, v tom případě se 
...
\flushpar Podle řešení kolize dělíme dál hašování:
\roster
\item"{}"
...
\heading
Hašování s přemís\v továním
\endheading

\flushpar Položky pro práci s tabulkou: next, previous\newline 
...
\flushpar Protože velikost tabulky omezuje velikost reprezentované množiny, může nastat přeplnění. O řešení 
...
\flushpar Příklad: $U=\{1,2,\dots,1000\}$, $h(x)=x\bmod10$,\newline 
...
\flushpar Tabulka vznikla následující posloupností 
operací:\newline 
...
\flushpar V příkladu provedeme {\bf INSERT$(28)$}, nový řádek je 4. řádek\newline 
...
\flushpar Očekávaný počet testů je stejný jako pro 
...
\flushpar Nevýhodou hašování s přemís\v továním je v operaci 
...
\flushpar Položky pro práci s tabulkou -- next, 
begin\newline 
...
\flushpar Stejná data jako v minulém případě\newline 

\flushpar Hašovací tabulka:
...
\flushpar V příkladu provedeme {\bf INSERT$(28)$}, nový řádek je 4. 
řádek\newline 
-- výsledná hašovací tabulka
$$\vtop{\offinterlineskip\halign {\strut\vrule\ # & \vrule\ # & \vrule\ # & \vrule \hfil # \hfil \vrule\cr\noalign{\hrule}řádek & key & next & begin\hfil \cr\noalign{\hrule}P(0) & & &\hfill \cr\noalign{\hrule}P(1) & 1 & 9 & 1 \cr\noalign{\hrule}P(2) & & &\hfill \cr\noalign{\hrule}P(3) & 73 & 7 & 3 \cr\noalign{\hrule}P(4) & 28 &  &\hfill \cr\noalign{\hrule}P(5) & 161 & & \hfill \cr\noalign{\hrule}P(6) & 7 & &\hfill \cr\noalign{\hrule}P(7) & 53 & & 6 \cr\noalign{\hrule}P(8) & 11 & 5 & 4  \cr\noalign{\hrule}P(9) & 141 & 8 & \hfill \cr\noalign{\hrule}}}$$

\flushpar Algoritmus při práci s položkami je rychlejší než při 
...
\flushpar Srůstající hašování se dělí podle práce s 
...
\flushpar Organizace tabulky je stejná jako v předchozích 
případech.\newline 
...
\flushpar Tabulka pro metodu LISCH vznikla následující posloupností 
operací:\newline 
{\bf INSERT$(1)$}, {\bf INSERT$(141)$}, {\bf INSERT$(11)$}, {\bf INSERT$
(73)$}, 
{\bf INSERT$(53)$},\newline 
...
\flushpar Provedeme {\bf INSERT$(28)$}, přidáváme do čvrtého rádku, vý\-sled\-ná tabulka vlevo je pro meto\-du 
...
\flushpar Algoritmus operace {\bf MEMBER} je pro obě metody stejný.
\bigskip
...
\flushpar Efektivní operace {\bf DELETE}  není známá, ale i primitivní 
...
\flushpar Popis situace: Uložena množina 
...
\flushpar Označení: $C(a_1,a_2,\dots,a_n;a_{n+1})$ označuje počet 
...
\flushpar Rekurence pro $S_n$ dává 
...
\flushpar Očekávaný počet testů při úspěšném 
...
\flushpar Základní idea: Metody používají pomocnou pamě\v t. 
...
\flushpar Idea: pomocná část má zabránit rychlému 
srůstání řetězců.
\medskip

\flushpar Tyto metody nepodporují přirozené efektivní algoritmy 
...
\flushpar Hašovací tabulka vznikla posloupnostmi operací:\newline 
Pro metodu LICH:\newline 
...
\flushpar ale nedodržovalo se, že se nejdřív zaplňují řádky z 
...
}\flushpar Algoritmus operace {\bf INSERT} je pro metodu LICH stejný 
...
\flushpar Složitost algoritmů pro srůstající hašování.\newline 
...
\flushpar Očekávaný počet testů pro metodu LICH\newline 
neúspěšný případ:\newline 
...
\flushpar Očekávaný počet testů pro metodu EICH\newline 
neúspěšný případ:\newline 
\phantom{---}$e^{-\frac {\alpha}{\beta}}+\frac {\alpha}{\beta}$, když  $
\alpha\le\lambda\beta$, \newline 
...
\flushpar Očekávaný počet testů pro metodu VICH\newline 
neúspěšný případ:\newline 
\phantom{---}$e^{-\frac {\alpha}{\beta}}+\frac {\alpha}{\beta}$, když  $
\alpha\le\lambda\beta$, \newline 
\phantom{---}$\frac 1{\beta}+\frac 14(e^{2(\frac {\alpha}{\beta}-
...
\flushpar Tabulka má jedinou položku -- key
\medskip

\flushpar Základní idea:  Při operaci {\bf INSERT$(x)$} vložíme $
x$ 
...
\flushpar Komentář:  Metoda vyžaduje minimální velikost paměti.  
...
\flushpar Příklad: Máme universum $U=\{1,2,\dots,1000\}$, 
...
\flushpar Tabulka vznikla posloupností operací:\newline 
{\bf INSERT$(1)$}, {\bf INSERT$(11)$}, {\bf INSERT$(73)$}, 
{\bf INSERT$(141)$}, {\bf INSERT$(161)$},\newline 
{\bf INSERT$(53)$}, {\bf INSERT$(7)$}. 

\flushpar Na závěr uvedeme složitost této metody. Očekávaný 
počet testů:\newline 
...
\flushpar Základní nevýhoda předchozí metody je způsob 
...
\flushpar Příklad: Mějme universum $U=\{1,2,\dots,1000
\}$. 
...
\flushpar Analýza vyhledávání v dvojitém hašování.
\medskip
\subhead
Neúspěšný případ
\endsubhead
...
\flushpar Značení:  $q_i(n,m)$ -- když tabulka má $m$ řádků a 
...
\flushpar Indukcí ukážeme, že $C(n,m)=\frac {m+1}{m-n
+1}$. Když $n=0$, pak 
...
\flushpar Použijeme metodu ze separujících řetězců. 
...
\flushpar Pořadí metod hašování podle očekávaného počtu 
testů:
\subhead
Neúspěšné vyhledávání
\endsubhead

\flushpar\phantom{---}Hašování s uspořádanými řetězci,\newline 
...
\flushpar\phantom{---}Hašování s uspořádanými řetězci=Hašování s 
...
\flushpar Poznámka: Metoda VICH při neúspěšném vyhledávání pro 
$\alpha <0.72$ a při úspěšném vy\-hledávání pro $
\alpha <0.92$ 
vyžaduje menší očekávaný počet testů než 
metoda s dvěma ukazateli.

\flushpar Při neúspěšném vyhledávání jsou metody VICH a LICH stejné a jsou o 8\% lepší než 
...
\flushpar\phantom{---}Metoda s přemís\v továním: neúspěšné vyhledávání 1.5, úspěšné 
vyhledávání 1.4.\newline 
...
\flushpar Metodu s lineárním přidáváním je dobré použít jen pro 
$\alpha <0.7$, metodu s dvojitým hašováním pro $\alpha 
<0.9$, pak 
čas pro neúspěšné vyhledávání rychle narůstá.
\medskip

\flushpar Vliv $\beta =\frac m{m'}$ při srůstajícím hašování.\newline 
Při 
...
\flushpar Komentář: Metody se separujícími řetězci a 
...
\flushpar Jak nalézt volný řádek.\newline 
...
\flushpar Jak řešit {\bf DELETE} v metodách, které ho 
nepodporují.\newline 
...
\v Rádek nebude v seznamu volných řádků, ale 
...
\flushpar Připomeňme si předpoklady pro předchozí uvedené 
výsledky o hašování:
\roster
\item
...
\flushpar Diskutujme splnitelnost těchto předpokladů.\newline 
...
\flushpar Závěr: Podmínky 1) a 2) můžeme splnit, 
když známe rozložení vstupních dat, můžeme 
dosáhnout ještě lepších výsledků.
\medskip

\flushpar Nevýhoda: Rozložení vstupních dat 
...
\flushpar\v Rešení navrhli Carter a Wegman (1977), když přišli s 
metodou univerzálního hašování, která obchází poža\-davek 3).  
To vedlo k novému rozsáhlému používání hašování.  
\medskip

\flushpar Základní idea: Místo jedné funkce máme 
...
\flushpar Modifikace ideje. Ověřování vlastností vyžaduje 
...
problematické. \v Rešením problému je použití indexové množiny. 
...
\flushpar Formálně: Nech\v t  $U$ je univerzum. Soubor  
...
\flushpar Jako ekvivalentní definici lze použít toto tvrzení: systém funkcí $
...
\flushpar Problémy: existence $c$-univerzálních 
systémů, \newline 
...
\flushpar Univerzum $U=\{0,1,\dots,N-1\}$ pro prvočíslo $N$,\newline 
$H=\{h_{a,b}\mid (a,b)\in U\times U\}$,\newline 
kde $h_{a,b}(x)=((ax+b)\bmod N)\bmod m$\newline 
(tj. indexová množina je $U\times U$ a její velikost je $
N^2$).
\smallskip
\flushpar Výhoda: funkce z množiny $H$ umíme rychle 
vyčíslit.
\smallskip

\flushpar Zvolme $x,y\in U$ taková, že $x\ne y$. Chceme nalézt 
$(a,b)\in U\times U$ takové, že $h_{a,b}(x)=h_{a,b}(y)$. 
\medskip

\flushpar Musí existovat $i\in \{0,1,\dots,m-1\}$ a 
...
\flushpar Závěr: pro každá $x,y\in U$ taková, že 
...
\flushpar Skutečně, pro každé $x,y\in U$, $x\ne y$, je počet 
...
\flushpar Závěr: Dokázali jsme existenci $c$-univerzálních 
...
\flushpar Předpoklad: $H=\{h_i\mid i\in I\}$ je $c$-univerzální systém funkcí\newline 
...
\flushpar Pro fixovanou množinu $S\subseteq U$ a pro fixované $
x\in U$ sečteme
...
\flushpar Protože $\delta_i(x,S)$ dává odhad na velikost řetězce $
h_i(x)$ při 
...
\flushpar Předpoklady: Je dána množina $S\subseteq U$, prvek $
x\in U$. 
Očeká\-va\-ná velikost $\delta_i(x,S)$ je $\mu$ a $t\ge 
1$.
\medskip

\flushpar Ukážeme pro $t>1$, že pravděpodobnost, že $
\delta_i(x,S)\ge t\mu$ 
...
\flushpar Označme $I'=\{i\in I\mid\delta_i(x,S)\ge t\mu \}$. Pak platí
$$\mu =\frac {\sum_{i\in I}\delta_i(x,S)}{|I|}>\frac {\sum_{i\in 
I'}\delta_i(x,S)}{|I|}\ge\frac {\sum_{i\in I'}t\mu}{|I|}=\frac {|
I'|}{|I|}t\mu$$
\flushpar Odtud
$$|I'|<\frac {|I|}t.$$
\flushpar Závěr: Pravděpodobnost, že $\delta_i(x,S)\ge 
t\mu$, je menší 
než $\frac 1t$, a odtud plyne požadované tvrzení. 
\medskip

\flushpar Poznámka: Toto tvrzení platí obecně a nazývá 
...
\flushpar Hlavní problém: Zajištění rovnoměrného rozdělení 
výběru $i$ z $I$.
\medskip

\flushpar Provedení výběru: Zak\'odovat indexy z množiny $
I$ do 
...
\flushpar Závada: Skutečný náhodný generátor pro 
rovnoměrné rozděle\-ní je prakticky nedosažitelný 
(některé fyzikální procesy). K dispozici je pouze 
pseudogenerátor. 
\medskip

\flushpar Jeho nevýhoda: \v Cím je $j$ větší, tím je 
posloupnost pravidelnější (tj. méně náhodná). 
\medskip

\flushpar Důsledky: Nalézt co nejmenší $c$-univerzální 
...
\flushpar Předpoklady: Nech\v t $U$ je univerzum velikosti $N$ a nech\v t 
$H=\{h_i\mid i\in I\}$ je $c$-univerzální systém funkcí hašujících do tabulky velikosti 
$m$. Můžeme předpoklá\-dat, že 
$$I=\{0,1,\dots,|I|-1\}.$$
\medskip

\flushpar Indukcí definujme množiny $U_0,U_1,\dots$ tak, že: 
$U_0=U$.\newline 
...
\flushpar Protože hašujeme do tabulky velikosti $m$, platí 
...
\flushpar Zkonstruujeme $c$-univerzální systém takový, 
...
\flushpar Nech\v t $p_1,p_2,\dots$ je rostoucí posloupnost všech 
...
\flushpar Nejprve si připomeneme známou větu o velikosti prvočísel 
...
\flushpar Tedy pro $i\ge 6$ platí $p_i<2i\ln i$.
\medskip

\flushpar Velikost indexové množiny $H_1$.  Indexová množina $
H_1$ je 
...
\flushpar Zvolme různá $x$ a $y$ z univerza $U$. 
Označme
...
\flushpar Odhad velikosti $G_1$. Když $(c,d,\ell )\in G_1$, pak 
...
\flushpar Odhad velikosti $G_2$. Označme 
...
\flushpar Abychom odhadli $(1+\frac m{p_{2t}})^2$, ukážeme si nejdřív 
pomocné lem\-ma.
...
\flushpar Nyní zkombinujeme Větu o odhadu velikosti prvočísel, 
...
\flushpar Z toho plyne, že $(1+\frac m{p_{2t}})^2\le 1.5^2=2.2
5$, a tedy 
...
\flushpar Shrnutí: Když $t\ge 6$ a $m\ln m\ln\ln m<N$, pak $
H_1$ je 
...
\flushpar Použít jiné metody na řešení kolizí než 
...
\flushpar Existuje $c$-univerzální hašovací systém pro 
...
\flushpar Použití \v Cebyševovy nerovnosti místo 
...
\flushpar Jak použít Markovou nerovnost a očekávanou 
...
\flushpar Je dána základní velikost tabulky $m$ a dále pro $
j=0,1,\dots$ 
...
\flushpar Jiné řešení kolizí je perfektní hašování. Idea 
je nalézt pro danou množinu hašovací funkci, která 
nevytváří kolize.
\medskip

\flushpar Nevýhoda: Metoda nepřipouští operaci {\bf INSERT }
...
\flushpar Zadání úlohy: Pro danou množinu $S\subseteq 
U$ chceme 
...
\flushpar Kompenzace: Nalezení hašovací funkce 
může spotřebovat více času. Provádí se jen na 
začátku úlohy. 
\medskip

\flushpar Uvedené požadavky motivují zavedení 
následujícího pojmu.\newline 
...
\flushpar Protože nevíme, zda taková $h$ existují, nejprve 
...
\flushpar Předpokládejme, že $H$  je $(N,m,n)$-perfektní 
systém pro $U=\{0,1,\dots,N-1\}$ a nejprve nalezneme dolní 
odhady na velikost $|H|$.
\medskip

\flushpar Mějme funkci $h$ z $U$ do množiny $\{0,1,\dots,m-
1\}$. 
...
n-1}<m\}.$$
\flushpar Vysvětlení: $h(S)=\{i_j\mid j=0,1,\dots,n-1\}$.
\medskip

\flushpar Toto číslo je maximální, když $|h^{-1}(
...
\flushpar Jiný odhad velikosti $(N,m,n)$-perfektního souboru.
\medskip

\flushpar Předpokládejme, že $H=\{h_1,\dots,h_t\}$ je 
...
\flushpar Mějme univerzum $U=\{0,1,\dots,N-1\}$ a soubor 
...
\flushpar Vysvětlení:  $m^n$ je počet všech funkcí z $
S$ do 
...
\flushpar Podmnožin $U$ velikosti $n$ je $\binom Nn$, tedy počet všech 
matic, které nereprezentují $(N,m,n)$-perfektní systém, je 
menší nebo roven  
$$\binom Nn(m^n-\prod_{i=0}^{n-1}(m-i))^tm^{(N-n)t}.$$

\flushpar Všech matic je $m^{Nt}$ a když 
$$\binom Nn(m^n-\prod_{i=0}^{n-1}(m-i))^tm^{(N-n)t}<m^{Nt},\tag *$$
\flushpar pak nutně existuje $(N,m,n)$-perfektní systém. 
...
\flushpar Existence $(N,m,n)$-perfektního souboru funkcí ale 
...
\flushpar Předpoklady:  $U=\{0,1,\dots,N-1\}$, kde $N$ je 
...
\flushpar Význam $b_i^k$: Hodnoty $b_i^k$ lze považovat za 
veličiny, které ukazují odchylku od perfektnosti. 
Všimněme si, že 
$$\text{\rm když }b_i^k\ge 2,\text{\rm\ pak }(b_i^k)^2-b_i^k\ge 
2,$$
\flushpar protože $a^2-a\ge 2$, když $a\ge 2$. Na druhou stranu 
...
\flushpar Nyní odhadneme výraz $\sum_{k=1}^{N-1}\big((\sum_{
...
\flushpar Tedy existuje $k$ takové, že $\sum_{i=0}^{m-1}(b_i^
k)^2\le 2\frac {n(n-1)}m+n$. 
\medskip

\flushpar Ukážeme, že existuje více než $\frac {N-
1}4$ takových 
$k$, že platí
...
\flushpar Když $m=n(n-1)+1$, pak prohledáním všech možností 
...
\flushpar Takto zkonstruované perfektní hašovací funkce 
nesplňují poža\-davek 2) (platí $m=\Theta (n^2)$).  Použijeme následující 
postup. 
\medskip

\flushpar1) Nalezneme $k$ takové, že pro $m=n$ platí 
...
\flushpar Tedy zkonstruovaná hašovací funkce splňuje požadavky 
1), 2) a 3), ale požadavek 4) není splněn. 
\medskip

\flushpar Mějme přirozené číslo $m$ a nech\v t $q$ je počet všech prvočísel 
...
\flushpar Mějme $S=\{s_1<s_2<\dots<s_n\}\subseteq U$. Označme 
...
\flushpar Test, zda funkce $\phi_p(x)=x\bmod p$ je perfektní 
pro $S$, 
...
\endroster
\flushpar Odhad očekávaného počtu neúspěšných kroků.\newline 
...
\flushpar Deterministický algoritmus nalezne nejmenší prvočíslo s 
...
\flushpar Nyní navrhneme postup na konstrukci perfektní 
...
\flushpar Výsledek: $f$ je perfektní hašovací funkce pro $
S$, protože 
...
\flushpar Výpočet $\phi_{q_0}$ vyžaduje čas $O(n^3\log 
n\log N)$. Výpočet $h_l$ 
...
\flushpar Lze použít i pravděpodobnostní algoritmy pro nalezení 
...
\flushpar Tuto metodu navrhli Fredman, Koml\'os a Szemerédi.
\medskip
...
\flushpar Předchozí hlavní konstrukce perfektní hašovací funkce 
...
\flushpar Stejně jako v předchozí sekci spočítáme  
...
\flushpar Proto existuje $i\in I$ takové, že $\sum_{j=0}^{m-
...
\flushpar Pro $m=\frac {cn(n-1)}2+1$ existuje deterministický algoritmus, 
...
+a}a.\endalign$$
\flushpar Pro hledání perfektní hašovací funkce opět použijeme 
...
\flushpar Když máme $a>1$ a $m=\frac {(c+a)n(n-1)}2+1$ 
...
\flushpar Další postup konstrukce perfektní hašovací funkce už 
...
\flushpar Jedna z velkých nevýhod perfektního hašovaní je 
...
\flushpar Nejprve uvedeme modifikaci výsledků z 
...
\flushpar Algoritmy předpokládají, že při operaci {\bf INSERT$
(x)$} prvek $x$ nepatří do $S$ a při operaci {\bf DELETE$(x)$} $
x$ je prvkem $S$. Velikost reprezentované množiny je $n$. 
\medskip

\flushpar Uvedu složitost této metody bez důkazu.
\medskip
...
\flushpar Toto zobecnění Fredman-Koml\'os-Szemerédiho metody navrhli 
Dietzfelbinger, Karlin, Mehl\-horn, Meyer auf der Heide, Rohnert 
a Tarjan.
\bigskip

\flushpar Další nevýhoda Fredman-Koml\'os-Szemerédiho metody:\newline 
...
\flushpar Dvojice $(X,E)$, kde $X$ je množina a $E$ je systém 
...
\flushpar Nyní popíšeme metodu, která je rozdělena do 
dvou kroků. Je dáno $S\subseteq U$ takové, že 
$|S|=n$.
\medskip

\flushpar Krok 1) Mějme $r$-hypergraf $(V,E)$, kde $|E|=n$.  Nalezneme 
zobrazení 
$$g:V@>>>\{0,1,\dots,n-1\}$$
takové, že funkce 
...
\flushpar Krok 2) Nalezneme $r$ funkcí $f_1,f_2,\dots,f_r:U@>>>
V$ takových, že 
$(V,E)$, kde 
...
\flushpar Autoři ukázali, že jejich algoritmus vyžaduje $
O(rn+|V|)$ 
...
\flushpar Pro praktické použití je problematická 
...
\flushpar Navržený postup je také znám pod názvem Faginův algoritmus. Tímto problémem se jako první asi zabýval Larsson.
\medskip

\flushpar\v Rešíme jiný problém -- uložení dat v externí 
pamě\v ti. Hlavní problém -- minimalizovat přístupy na 
externí pamě\v t. 
\medskip

\flushpar Předpoklady:  Externí pamě\v t je rozdělena na 
...
\flushpar Podobný problém se také řeší pri práci s cache-pamětí. V tom případě však 
...
\flushpar Předpokládejme, že $h:U@>>>\{0,1\}^{*}$ je prosté 
...
.$$
\v Rekneme, že $\alpha$ je \emph{kritické} \emph{slovo}, když 
...
\flushpar Problém: jak nalézt stránku kritického slova 
$\alpha$?\newline 
\v Rešení: Adresář je funkce, která každému slovu $
\alpha$ o 
...
\endroster
\flushpar Korektnost: Pro různá kritická slova 
...
\flushpar Mějme slovo $\alpha$ o délce $d(S)$. Označme $c
(\alpha )$ nejkratší 
...
\flushpar Lineární uspořádání na slovech délky $
n$ nazveme 
...
\flushpar Uvedeme zde jen slovní popis operací. Předpokládáme, že adresář 
...
\flushpar Následující věta ukazuje, že jsme náš hlavní cíl splnili. Pro jednoduchost předpokládáme, že 
...
\flushpar V našem příkladu provedeme operaci 
...
\flushpar Vzniká otázka, jak je tato metoda efektivní. 
...
\flushpar První tvrzení říká, že očekávaný počet 
...
\flushpar kde jednotlivé řádky odpovídají hodnotám $
b$ 
